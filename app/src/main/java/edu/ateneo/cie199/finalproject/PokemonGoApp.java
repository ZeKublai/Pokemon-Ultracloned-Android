...
public class PokemonGoApp extends Application{
    ...
    public void loadAllPokemon(){
        addPokemon(new Pokemon(1, "Bulbasaur", "It can go for days without eating a single morsel. In the bulb on its back, it stores energy.", new StatSet(45, 49, 49, 65, 65, 45), mTypes.get(Type.GRASS), R.drawable.bulbasaur_main, R.drawable.bulbasaur_map, R.drawable.bulbasaur_back, 190, 7, 1));
        addPokemon(new Pokemon(2, "Ivysaur", "The bulb on its back grows by drawing energy. It gives off an aroma when it is ready to bloom.", new StatSet(60, 62, 63, 80, 80, 60), mTypes.get(Type.GRASS), R.drawable.ivysaur_main, R.drawable.ivysaur_map, R.drawable.ivysaur_back, 45, 7, 1));
        addPokemon(new Pokemon(3, "Venusaur", "The flower on its back catches the sun's rays. The sunlight is then absorbed and used for energy.", new StatSet(80, 82, 83, 100, 100, 80), mTypes.get(Type.GRASS), R.drawable.venusaur_main, R.drawable.venusaur_map, R.drawable.venusaur_back, 45, 7, 1));
        addPokemon(new Pokemon(4, "Charmander", "The flame at the tip of its tail makes a sound as it burns. You can only hear it in quiet places.", new StatSet(39, 52, 43, 60, 50, 65), mTypes.get(Type.FIRE), R.drawable.charmander_main, R.drawable.charmander_map, R.drawable.charmander_back, 190, 7, 1));
        addPokemon(new Pokemon(5, "Charmeleon", "It is very hotheaded by nature, so it constantly seeks opponents. It calms down only when it wins.", new StatSet(58, 64, 58, 80, 65, 80), mTypes.get(Type.FIRE), R.drawable.charmeleon_main, R.drawable.charmeleon_map, R.drawable.charmeleon_back, 45, 7, 1));
        addPokemon(new Pokemon(6, "Charizard", "When expelling a blast of super hot fire, the red flame at the tip of its tail burns more intensely.", new StatSet(78, 84, 78, 109, 85, 100), mTypes.get(Type.FIRE), R.drawable.charizard_main, R.drawable.charizard_map, R.drawable.charizard_back, 45, 7, 1));
        addPokemon(new Pokemon(7, "Squirtle", "Shoots water at prey while in the water. Withdraws into its shell when in danger.", new StatSet(44, 48, 65, 50, 64, 43), mTypes.get(Type.WATER), R.drawable.squirtle_main, R.drawable.squirtle_map, R.drawable.squirtle_back, 190, 7, 1));
        addPokemon(new Pokemon(8, "Wartortle", "Often hides in water to stalk unwary prey. For swimming fast, it moves its ears to maintain balance.", new StatSet(59, 63, 80, 65, 80, 58), mTypes.get(Type.WATER), R.drawable.wartortle_main, R.drawable.wartortle_map, R.drawable.wartortle_back, 45, 7, 1));
        addPokemon(new Pokemon(9, "Blastoise", "It deliberately makes itself heavy so it can withstand the recoil of the water jets it fires.", new StatSet(79, 103, 120, 135, 115, 78), mTypes.get(Type.WATER), R.drawable.blastoise_main, R.drawable.blastoise_map, R.drawable.blastoise_back, 45, 7, 1));
        addPokemon(new Pokemon(25, "Pikachu", "It keeps its tail raised to monitor its surroundings. If you yank its tail, it will try to bite you.", new StatSet(35, 55, 40, 50, 50, 90), mTypes.get(Type.ELECTRIC), R.drawable.pikachu_main, R.drawable.pikachu_map, R.drawable.pikachu_back, 190, 1, 1));
        addPokemon(new Pokemon(26, "Raichu", "When electricity builds up inside its body, it becomes feisty. It also glows in the dark.", new StatSet(60, 90, 55, 90, 80, 110), mTypes.get(Type.ELECTRIC), R.drawable.raichu_main, R.drawable.raichu_map, R.drawable.raichu_back, 75, 1, 1));
        addPokemon(new Pokemon(58, "Growlithe", "It has a brave and trustworthy nature. It fearlessly stands up to bigger and stronger foes.", new StatSet(55, 70, 45, 70, 50, 60), mTypes.get(Type.FIRE), R.drawable.growlithe_main, R.drawable.growlithe_map, R.drawable.growlithe_back, 190, 3, 1));
        addPokemon(new Pokemon(59, "Arcanine", "An ancient picture scroll shows that people were attracted to its movement as it ran through prairies.", new StatSet(90, 110, 80, 100, 80, 95), mTypes.get(Type.FIRE), R.drawable.arcanine_main, R.drawable.arcanine_map, R.drawable.arcanine_back, 75, 3, 1));
        addPokemon(new Pokemon(72, "Tentacool", "It can sometimes be found all dry and shriveled up on a beach. Toss it back into the sea to revive it.", new StatSet(40, 40, 35, 50, 100, 70), mTypes.get(Type.WATER), R.drawable.tentacool_main, R.drawable.tentacool_map, R.drawable.tentacool_back, 190, 1, 1));
        addPokemon(new Pokemon(73, "Tentacruel", "The tentacles are normally kept short. On hunts, they are extended to ensnare and immobilize prey", new StatSet(80, 70, 65, 80, 120, 100), mTypes.get(Type.WATER), R.drawable.tentacruel_main, R.drawable.tentacruel_map, R.drawable.tentacruel_back, 60, 1, 1));
        addPokemon(new Pokemon(74, "Geodude", "Commonly found near mountain trails, etc. If you step on one by accident, it gets angry.", new StatSet(40, 80, 100, 30, 30, 20), mTypes.get(Type.ROCK), R.drawable.geodude_main, R.drawable.geodude_map, R.drawable.geodude_back, 255, 1, 1));
        addPokemon(new Pokemon(75, "Graveler", "Often seen rolling down mountain trails. Obstacles are just things to roll straight over, not avoid.", new StatSet(55, 95, 115, 45, 45, 35), mTypes.get(Type.ROCK), R.drawable.graveler_main, R.drawable.graveler_map, R.drawable.graveler_back, 120, 1, 1));
        addPokemon(new Pokemon(76, "Golem", "Its boulder-like body is extremely hard. It can easily withstand dynamite blasts without damage.", new StatSet(80, 120, 130, 55, 65, 45), mTypes.get(Type.ROCK), R.drawable.golem_main, R.drawable.golem_map, R.drawable.golem_back, 45, 1, 1));
        addPokemon(new Pokemon(127, "Pinsir", "Grips its prey in its pincers and squeezes hard! It can't move if it's cold, so it lives in warm places", new StatSet(65, 125, 100, 55, 70, 85), mTypes.get(Type.BUG), R.drawable.pinsir_main, R.drawable.pinsir_map, R.drawable.pinsir_back, 45, 1, 1));
        addPokemon(new Pokemon(128, "Tauros", "They fight each other by locking horns. The herd's protector takes pride in its battle-scarred horns.", new StatSet(75, 100, 95, 40, 70, 110), mTypes.get(Type.NORMAL), R.drawable.tauros_main, R.drawable.tauros_map, R.drawable.tauros_back, 45, 1, 0));
    }
